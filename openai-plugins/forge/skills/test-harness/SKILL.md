...
as tools, so the active coding agent can drive the application headlessly: launch it,
send inputs, read state, find bugs. This skill builds the harness.
The QA pass is a separate task the user starts afterward.

Focus area: the user's request. Empty means the full public API surface.
...
- An `mcpServers` entry in `.mcp.json` at the repo root, merged into
  ...
  The active host may ask to approve the project server on next start.
